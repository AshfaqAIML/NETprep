...
export async function GET() {
  try {
    ...
    const p1Pyqs = await db.question.findMany({
      where: { sourceType: { in: ['official_pyq', 'verified_pyq'] }, paper: 'I', pyqYear: { gt: 0 } },
    ...
    const p2Pyqs = await db.question.findMany({
      where: { sourceType: { in: ['official_pyq', 'verified_pyq'] }, paper: 'II', pyqYear: { gt: 0 } },
      include: { topic: { include: { unit: true } } },
    })
    ...
    const allPyqTopics = await db.topic.findMany({
      where: {
        questions: {
          some: { sourceType: { in: ['official_pyq', 'verified_pyq'] }, pyqYear: { gt: 0 } },
        },
      },
      include: {
